...
func place_zones():
	var fill_rect_left_x = fill_rect.global_position.x
	var fill_rect_right_x = fill_rect.global_position.x + fill_rect.size.x
	optimal_zone.global_position.x = randi_range(fill_rect_left_x + ZONE_WIDTH / 2 + ZONE_MOVE_OFFSET, fill_rect_right_x - ZONE_WIDTH / 2 - ZONE_MOVE_OFFSET)
...
func handle_random_zone_movements(num_movements, max_movements):	
	...
	var fill_rect_left_x = fill_rect.global_position.x
	var fill_rect_right_x = fill_rect.global_position.x + fill_rect.size.x
	var rand_x = -ZONE_MOVE_OFFSET if randi_range(0, 1) == 0 else ZONE_MOVE_OFFSET
	var pos_to_tween_to = clamp(optimal_zone.global_position.x + rand_x, fill_rect_left_x, fill_rect_right_x)
